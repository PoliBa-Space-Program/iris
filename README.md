...
#![no_std]
pub mod utils {
    pub fn to_be_bytes_u8(a: &[u8]) -> Result<u8, &str> {
        if a.len() != 1 {
            Err("Invalid slice length. Expected length 1.")
        }
        else {
            Ok(a[0])
        }
    }
    pub fn to_be_bytes_u16(a: &[u8]) -> Result<u16, &str> {
        if a.len() != 2 {
            Err("Invalid slice length. Expected length 2.")
        }
        else {
            Ok(
                ((a[0] as u16) << 8) | 
    ...
    pub fn to_be_bytes_u32(a: &[u8]) -> Result<u32, &str> {
        if a.len() != 2 {
            Err("Invalid slice length. Expected length 4.")
        }
        else {
            Ok(
                ((a[0] as u32) << 32) | 
    ...
    pub fn to_be_bytes_i8(a: &[u8]) -> Result<i8, &str> {
        if a.len() != 1 {
            Err("Invalid slice length. Expected length 1.")
        }
        else {
    ...
    pub fn to_be_bytes_i16(a: &[u8]) -> Result<i16, &str> {
        if a.len() != 2 {
            Err("Invalid slice length. Expected length 2.")
        }
        else {
    ...
    pub fn to_be_bytes_i32(a: &[u8]) -> Result<i32, &str> {
        if a.len() != 2 {
            Err("Invalid slice length. Expected length 4.")
        }
        else {
    ...
    pub fn to_be_bytes_f32(a: &[u8]) -> Result<f32, &str> {
        if a.len() != 2 {
            Err("Invalid slice length. Expected length 4.")
        }
        else {
    ...
    pub fn to_be_bytes_bool(a: &[u8]) -> Result<bool, &str> {
        if a.len() != 1 {
            Err("Invalid slice length. Expected length 1.")
        }
        else {
...
pub mod iris {
    pub mod Telemetry {
        pub mod FlighData {
            const FLIGHT_DATA_NAME_HASH: u32 = 6161343;
            const FLIGHT_DATA_LENGTH_BYTES: u32 = 32 + 4;
            ...
            impl FlightData {
                pub fn encode(max_altitude: f32, max_velocity: f32, computer_id: u32, current_altitude: f32, mesured_temperatures: &[f32]) -> [u8; FLIGHT_DATA_LENGTH_BYTES] {
                    let mut data: [u8; FLIGHT_DATA_LENGTH_BYTES] = [0; FLIGHT_DATA_LENGTH_BYTES];
                    let mut index = 0;

                    for x in FLIGHT_DATA_NAME_HASH.to_be_bytes() {
                        data[index] = x;
                        index += 1;
                    }

                    for x in max_altitude.to_be_bytes() {
                        data[index] = x;
                        index += 1;
                    }

                    for x in max_velocity.to_be_bytes() {
                        data[index] = x;
                        index += 1;
                    }

                    for x in computer_id.to_be_bytes() {
                        data[index] = x;
                        index += 1;
                    }

                    for x in current_altitude.to_be_bytes() {
                        data[index] = x;
                        index += 1;
                    }

                    for i in mesured_temperatures {
