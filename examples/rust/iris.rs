...
let mut data: [u8; Computer::BYTES_LENGTH] = [0; Computer::BYTES_LENGTH];let mut index = 0;for x in u32::to_be_bytes(Computer::NAME_HASH) {
data[index] = x;
index += 1;
}
...
let mut data: [u8; Computer::BYTES_LENGTH - 4] = [0; Computer::BYTES_LENGTH - 4];let mut index = 0;for x in self.id.to_be_bytes() {
data[index] = x;
index += 1;
}
...
index += 4;
for i in 0..2 {
out.batteries[i] = Battery::from_be_bytes(data[index..index+4].try_into().unwrap());
index += 4;
}
...
let mut data: [u8; Data::BYTES_LENGTH] = [0; Data::BYTES_LENGTH];let mut index = 0;for x in u32::to_be_bytes(Data::NAME_HASH) {
data[index] = x;
index += 1;
}
for x in self.to_be_bytes() {
...
let mut data: [u8; Data::BYTES_LENGTH - 4] = [0; Data::BYTES_LENGTH - 4];let mut index = 0;for i in self.computers {
for x in i.to_be_bytes() {
data[index] = x;
index += 1;
}
...
let mut index = 0;
for i in 0..2 {
out.computers[i] = Computer::from_be_bytes(data[index..index+16].try_into().unwrap());
index += 16;
}
out
}
...
let mut data: [u8; Battery::BYTES_LENGTH] = [0; Battery::BYTES_LENGTH];let mut index = 0;for x in u32::to_be_bytes(Battery::NAME_HASH) {
data[index] = x;
index += 1;
}
for x in self.to_be_bytes() {
...
let mut data: [u8; Battery::BYTES_LENGTH - 4] = [0; Battery::BYTES_LENGTH - 4];let mut index = 0;for x in self.charge.to_be_bytes() {
data[index] = x;
index += 1;
}
data
...
packages::Telemetry::Computer::NAME_HASH if data.len() == packages::Telemetry::Computer::BYTES_LENGTH => Ok(Structs::Telemetry_Computer(packages::Telemetry::Computer::decode(&data))),
packages::Telemetry::Data::NAME_HASH if data.len() == packages::Telemetry::Data::BYTES_LENGTH => Ok(Structs::Telemetry_Data(packages::Telemetry::Data::decode(&data))),
packages::Telemetry::Battery::NAME_HASH if data.len() == packages::Telemetry::Battery::BYTES_LENGTH => Ok(Structs::Telemetry_Battery(packages::Telemetry::Battery::decode(&data))),
_ => Err("Unknown data.")
}
}
}
