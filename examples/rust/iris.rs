...
let mut data: [u8; FlightData::BYTES_LENGTH] = [0; FlightData::BYTES_LENGTH];let mut index = 0;for x in u32::to_be_bytes(FlightData::NAME_HASH) {
data[index] = x;
index += 1;
}
...
index += 1;
out.current_altitude = f32::from_be_bytes(data[index..index+4].try_into().unwrap());
index += 4;
for i in 0..4 {
...
Telemetry::FlightData::NAME_HASH if data.len() == Telemetry::FlightData::BYTES_LENGTH => Ok(Structs::Telemetry_FlightData(Telemetry::FlightData::decode(&data))),
_ => Err("Unknown data.")
}
}
}
