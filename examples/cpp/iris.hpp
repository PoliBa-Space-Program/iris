...
auto is_le = []() { int a = 1; return 1 == (int)*((byte*)(&a)) ? true : false; };
template <typename T>
...
Battery batteries[2];
Computer() {}
Computer(unsigned int id,Status status,Battery *batteries) {
this->id = id;
this->status = status;
for (int i = 0; i < 2; i++) { this->batteries[i] = batteries[i]; }
}
iris::byte *encode() {
iris::to_be_bytes(this->NAME_HASH(), this->DATA_BUFFER);
this->to_be_bytes();
...
for (int i = 0; i < 2; i++) { this->computers[i] = computers[i]; }
}
iris::byte *encode() {
iris::to_be_bytes(this->NAME_HASH(), this->DATA_BUFFER);
this->to_be_bytes();
...
if (struct_name_hash == packages::Telemetry::Battery::NAME_HASH() && len == packages::Telemetry::Battery::BYTES_LENGTH()) { return T::decode(raw); }
else if (struct_name_hash == packages::Telemetry::Computer::NAME_HASH() && len == packages::Telemetry::Computer::BYTES_LENGTH()) { return T::decode(raw); }
else if (struct_name_hash == packages::Telemetry::Data::NAME_HASH() && len == packages::Telemetry::Data::BYTES_LENGTH()) { return T::decode(raw); }
else { throw 1; }
}
enum Structs {
Telemetry_Battery,
...
unsigned int struct_name_hash = from_be_bytes<unsigned int>(raw);
if (struct_name_hash == packages::Telemetry::Battery::NAME_HASH() && len == packages::Telemetry::Battery::BYTES_LENGTH()) { return Structs::Telemetry_Battery; }
else if (struct_name_hash == packages::Telemetry::Computer::NAME_HASH() && len == packages::Telemetry::Computer::BYTES_LENGTH()) { return Structs::Telemetry_Computer; }
else if (struct_name_hash == packages::Telemetry::Data::NAME_HASH() && len == packages::Telemetry::Data::BYTES_LENGTH()) { return Structs::Telemetry_Data; }
else { throw 1; }
}
}
